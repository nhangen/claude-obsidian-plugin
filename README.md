...
- Keeper containment and atomic publication cover cooperating same-host writers using the canonical vault lock, including commit/session capture, watcher quarantine, Base, Librarian, Pending, INDEX, INDEX state, and daily backlink writes. Ownerless locks are reclaimed only after a bounded stale interval; an interrupted reclaim removes the canonical lock path before cleanup, so it cannot permanently wedge later writers. The portable shell implementation rejects symlink components and revalidates immediately before rename, but it does not claim protection from an unrelated process replacing filesystem components outside that lock.
  - The MCP stdio and Streamable HTTP transports expose `obsidian_keeper_save` and `obsidian_daily_append` behind `vault:write`; HTTP writes require authentication and an idempotency key. Keeper-save callers must provide `resolved: true` and a nonempty caller-resolved vault-relative folder, and keyless ambiguous or partial outcomes require manual verification before retry.
