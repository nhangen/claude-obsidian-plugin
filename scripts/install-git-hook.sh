...
is_managed_hook() {
  local hook_file="$1" line1 line2 line3 line4
  ...
  [ "$line1" = "#!/usr/bin/env bash" ] \
    && [ "$line2" = "$MARKER" ] \
    && [ "$line3" = "# Managed post-commit hook for Obsidian vault commit capture." ] \
    && [ "$line4" = "# Automatically generated by install-git-hook.sh." ]
}
...
cmd_install() {
  ...
  if [ -f "$hook_file" ] && ! is_managed_hook "$hook_file"; then
    if [ -f "$legacy_file" ]; then
